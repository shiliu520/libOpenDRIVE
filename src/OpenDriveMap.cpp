                           ...
                           const bool         exclude_non_driving_lanes) : xodr_file(xodr_file)
{
    ...
    if (auto geoReference_node = odr_node.child("header").child("geoReference"))
        proj4 = geoReference_node.text().as_string("");
    ...
    for (pugi::xml_node junction_node : odr_node.children("junction"))
    {
        ...
        Junction& junction = id_to_junction.insert({junction_id, Junction(junction_node.attribute("name").as_string(""), junction_id)}).first->second;
    ...
    const auto& roads = odr_node.children("road");

    for (pugi::xml_node road_node : roads)
    {
        // #pragma omp task
        {
            /* make road */
            std::string road_id = road_node.attribute("id").as_string("");
            CHECK_AND_REPAIR(id_to_road.find(road_id) == id_to_road.end(),
            ...
            for (pugi::xml_node geometry_hdr_node : road_node.child("planView").children("geometry"))
            {
                ...
                double x0 = geometry_hdr_node.attribute("x").as_double(0.0) - x_offs;
                double y0 = geometry_hdr_node.attribute("y").as_double(0.0) - y_offs;
            ...
            for (pugi::xml_node lanesection_node : road_node.child("lanes").children("laneSection"))
            {
                ...
                for (pugi::xpath_node lane_xpath_node : lanesection_node.select_nodes(".//lane"))
                {
                    ...
                    if (exclude_non_driving_lanes && !(std::string(lane_node.attribute("type").as_string("")) == "driving" ||
                                                       std::string(lane_node.attribute("type").as_string("")) == "border" ||
                                                       std::string(lane_node.attribute("type").as_string("")) == "restricted" || lane_id == 0))
                        continue;
...
std::vector<Road> OpenDriveMap::get_roads() const { return get_map_values(id_to_road); }

std::vector<Junction> OpenDriveMap::get_junctions() const { return get_map_values(id_to_junction); }
    ...
    for (const auto& id_road : id_to_road)
    {
        ...
        for (auto s_lanesec_iter = road.s_to_lanesection.begin(); s_lanesec_iter != road.s_to_lanesection.end(); s_lanesec_iter++)
        {
            ...
            for (const auto& id_lane : lanesec.id_to_lane)
            {
                ...
                {
                    predecessor = get_connecting_lane(lane, true, prev_lanesection);
                    successor = get_connecting_lane(lane, false, next_lanesection);
                }
                else
                {
...
std::optional<Lane> OpenDriveMap::get_connecting_lane(const Lane& lane, bool predecessors, std::optional<LaneSection> target_lanesection) const
{
...
std::optional<LaneSection>
OpenDriveMap::adjacent_lanesection(const Road& current_road, const LaneSection& current_lanesection, bool predecessors) const
{
    // Either get lanesection on same road
    std::optional<LaneSection> lanesection =
        predecessors ? current_road.get_previous_lanesection(current_lanesection) : current_road.get_next_lanesection(current_lanesection);

    // If no next lanesection on same road, get from connecting road
    if (!lanesection)
